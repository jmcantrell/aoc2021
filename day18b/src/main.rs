...
fn find_largest_magnitude_sum(roots: &[TreeNode], n: usize) -> usize {
    roots
        .iter()
        .permutations(n)
...
fn main() {
    ...
    let roots = parse_trees(&s);
    dbg!(find_largest_magnitude_sum(&roots, 2));
}
...
mod tests {
    ...
    fn test_example() {
        ...
        let roots = parse_trees(&s);
        assert_eq!(find_largest_magnitude_sum(&roots, 2), 3993);
    }
}
